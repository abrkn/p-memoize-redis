...
module.exports = function createRedisMemCache(redisUrl) {
  const redisClient = redis.createClient(redisUrl);

  const asBase64 = value =>
    new Buffer(JSON.stringify(value)).toString('base64');
  const withPrefix = key => `${PREFIX}${asBase64(key)}`;
  const has = key => redisClient.existsAsync(withPrefix(key));
  const get = key =>
    redisClient.getAsync(withPrefix(key)).then(_ => (_ ? JSON.parse(_) : _));
  ...
  const clear = () =>
    getAllKeys.then(keys => Promise.all(key => redisClient.delAsync(key)));
